...
class TransferFragment : Fragment() {
        ...
        transferButton.setOnClickListener {
            ApplicationDatabase.getAppDatabase(requireContext())
                ?.getDAO()?.saveTransfer(TransferEntity(
                    userId = "23213135",
                    userName = "Motor",
                    transactionDate = "12/12/23",
                    transactionAmount = "50.000",
                    receiverUserId = "92131"
                ))
            val runnable = Runnable {
                var userTransferString = ""
                val transferList = ApplicationDatabase.getAppDatabase(requireContext())
                    ?.getDAO()?.findTransferByUserName("Motor")
                transferList!!.forEach { userTransferString += "\n" + it }
                Toast.makeText(requireContext(), userTransferString, Toast.LENGTH_LONG)
            }
            val handler = Handler()
    ...
    private fun initRecyclerView() {
        ...
        val transferAccount = TransferAccount.Builder("12345")
            .setProfilePhotoUrl("https://www.google.com/imagenDePrueba")
